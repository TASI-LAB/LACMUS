...
class VectorQuantization(Function):
    ...
    def forward(ctx, inputs, codeBook):
        with torch.no_grad():
            ...
            # print(f"codeBook_sqr: {codeBook_sqr.shape}; inputs_sqr: {inputs_sqr.shape}; inputs_flatten: {inputs_flatten.shape}; codeBook: {codeBook.size()}")
            # exit()
            # Compute the distances of the inputs to the codeBook
            distances = torch.addmm(codeBook_sqr + inputs_sqr,
                                    inputs_flatten, codeBook.t(), alpha=-2.0, beta=1.0)
            
            distance, indices_flatten = torch.min(distances, dim=1)
            indices = indices_flatten.view(*inputs_size[:-1])
            
            ctx.mark_non_differentiable(indices)

            return indices, distance.view(*inputs_size[:-1])
...
class VectorQuantizationStraightThrough(Function):
    ...
    def forward(ctx, inputs, codeBook):
        ...
        indices,distance = vq(inputs, codeBook)
        ...
        return codes, indices_flatten,distance
